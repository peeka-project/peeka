...
import peeka.cli.main as cli_main
from peeka.cli.handlers import consumers as cli_consumers
...
class TestConsumerCLICreate:
    def test_consumer_create_emits_json_envelope(self, monkeypatch, capsys):
        ...
        exit_code = cli_consumers.cmd_consumer(args)
...
class TestConsumerCLIStatus:
    ...
    def test_consumer_status_passes_client_session_id(self, monkeypatch, capsys):
        ...
        _ = cli_consumers.cmd_consumer(args)
        _ = capsys.readouterr()
        assert captured_command["client_session_id"] == "client_1"


class TestConsumerCLIDrain:
    def test_consumer_drain_emits_envelope_then_jsonl_records(
    ...
    def test_consumer_close_and_cleanup(self, monkeypatch, capsys):
        class MockStreamingAgentClientClose:
            ...
            def send_command(self, command: Dict[str, Any]) -> Dict[str, Any]:
                return {
                    "status": "success",
                    "data": {"closed": True, "consumer_id": "consumer_a"},
                }
        ...
        monkeypatch.setattr(cli_connection, "StreamingAgentClient", MockStreamingAgentClientClose)
        ...
        exit_code = cli_consumers.cmd_consumer(close_args)
        captured = capsys.readouterr()
        assert exit_code == 0
        assert json.loads(captured.out.strip())["command"] == "consumer.close"

        monkeypatch.setattr(cli_connection, "StreamingAgentClient", MockStreamingAgentClientCleanup)
        ...
        exit_code = cli_consumers.cmd_consumer(cleanup_args)
        captured = capsys.readouterr()
        assert exit_code == 0

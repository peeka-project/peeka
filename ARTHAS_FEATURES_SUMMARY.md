...
```bash
peeka watch <pid> "demo.Calculator.add" -b
peeka watch <pid> "demo.Calculator.divide" -e
peeka watch <pid> "demo.slow_operation" --condition-express 'cost > 15'
```
...
```bash
peeka watch <pid> "module.Class.method" -b
```
**Output**:
```json
{
...
```bash
peeka watch <pid> "module.Class.method" -s
```
...
```bash
peeka watch <pid> "module.Class.method" -e
```
...
### Cost Filtering
```bash
peeka watch <pid> "module.slow_func" --condition-express "cost > 50"
```
Only observes calls taking more than 50ms.

### Combined Flags
```bash
peeka watch <pid> "module.func" -b -s
```

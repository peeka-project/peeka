...
```bash
# 每 60 秒输出一次统计数据，持续运行
peeka monitor -p 12345 "myapp.api.handle_request" --interval 60
```
...
peeka monitor -p 12345 "myapp.payment.process" \
  --interval 30 -c 10 | \
...
```bash
# 监控 1 小时（60 次，每分钟一次）
peeka monitor -p 12345 "myapp.db.execute_query" \
...
```bash
# 终端 1：监控 API v1
peeka monitor -p 12345 "myapp.api.v1.handler" --interval 30 > api_v1.jsonl

# 终端 2：监控 API v2
peeka monitor -p 12345 "myapp.api.v2.handler" --interval 30 > api_v2.jsonl
...
```bash
# 监控核心函数，每 10 秒输出一次
peeka monitor -p 12345 "myapp.process" --interval 10 | \
...
```bash
peeka monitor [--pid PID | --name NAME] <pattern> [options]
```
...
```bash
# 高频监控（每 10 秒）
peeka monitor -p 12345 "myapp.api.handler" --interval 10

# 长期监控（每 5 分钟）
peeka monitor -p 12345 "myapp.batch.process" --interval 300
```
...
peeka monitor -p 12345 "myapp.func" --interval 60 -c 1

# 监控 10 分钟（10 次，每次 1 分钟）
peeka monitor -p 12345 "myapp.func" --interval 60 -c 10

# 持续监控（直到手动停止）
peeka monitor -p 12345 "myapp.func" --interval 60
```
...
```bash
peeka monitor -p 12345 "myapp.payment.charge" --interval 60 -c 5
```
...
```bash
peeka monitor -p 12345 "myapp.critical_func" --interval 30 | \
...
peeka monitor -p 12345 "myapp.db.query" --interval 60 -c 30 | \
...
```bash
# 步骤 1：启动监控，发现响应时间异常
peeka monitor -p 12345 "myapp.process" --interval 60 | \
  jq -r 'select(.rt_avg > 100)'
# 输出：{"cycle": 5, "rt_avg": 234.567, ...}

# 步骤 2：使用 watch 查看详细调用信息
peeka watch -p 12345 "myapp.process" -n 10
# 分析参数、返回值、执行时间
...
```bash
# 步骤 1：监控核心函数 1 小时
peeka monitor -p 12345 "myapp.api.handle_request" \
...
```bash
# 步骤 1：同时监控两个函数
peeka monitor -p 12345 "myapp.api.v1.handler" --interval 30 > v1.jsonl &
peeka monitor -p 12345 "myapp.api.v2.handler" --interval 30 > v2.jsonl &
...
```bash
# 步骤 1：启动监控（高频，每 10 秒）
peeka monitor -p 12345 "myapp.process" --interval 10 > load_test.jsonl &
...
```bash
# 终端 1：监控 API
peeka monitor -p 12345 "myapp.api.handler" --interval 60

# 终端 2：监控数据库
peeka monitor -p 12345 "myapp.db.query" --interval 60

# 终端 3：监控缓存
peeka monitor -p 12345 "myapp.cache.get" --interval 60
```
...
```bash
peeka monitor -p 12345 --action status
```

**方法 2**：检查进程是否有对应的客户端连接
```bash
ps aux | grep "peeka monitor" | grep 12345
```
...
```bash
peeka monitor -p 12345 "myapp.async_handler" --interval 60
```
...
peeka monitor -p 12345 "json.dumps" --interval 10 -c 6
```
...
# 启动监控（后台）
peeka monitor -p $PID "$PATTERN" --interval 10 > $LOG &
MONITOR_PID=$!
...
peeka monitor -p $PID "$PATTERN" --interval 60 | \
  jq -r '
...
# 监控 5 分钟
current_rt=$(peeka monitor -p $PID "$PATTERN" --interval 60 -c 5 | \
...
```bash
# 监控 3 个函数（并行）
peeka monitor -p 12345 "myapp.api.v1" --interval 60 -c 10 > v1.jsonl &
peeka monitor -p 12345 "myapp.api.v2" --interval 60 -c 10 > v2.jsonl &
peeka monitor -p 12345 "myapp.api.v3" --interval 60 -c 10 > v3.jsonl &
...
# 收集 1 周的监控数据
for day in {1..7}; do
  peeka monitor -p 12345 "myapp.func" --interval 3600 -c 24 > \
...
|------|---------------|----------------|------|
| 定期统计输出 | ✅ `monitor` | ✅ `monitor` | 一致 |
| 调用次数统计 | ✅ | ✅ | 一致 |
| 成功/失败统计 | ✅ | ✅ | 一致 |
| 响应时间统计 | ✅ | ✅ | 一致 |
| 自定义输出间隔 | ✅ `-c` | ✅ `--interval` | 参数名不同 |
| 限制周期数 | ✅ `-n` | ✅ `--cycles` | 参数名不同 |
| 条件过滤 | ✅ | ❌ | Arthas 支持条件过滤 |
...
**Arthas (Java)**：
```bash
monitor com.example.MyClass myMethod -c 5 -n 10
```
- `-c 5`：每 5 秒输出一次
- `-n 10`：监控 10 次
...
**Peeka (Python)**：
```bash
peeka monitor -p 12345 "myapp.MyClass.myMethod" --interval 5 -c 10
```
...
----------------------------------------------------------------------------------------------
 2024-01-01 com.example.  myMethod 100    95       5     45.67       5.00%
```

**Peeka (JSON)**：
```json
{"cycle":1,"total":100,"success":95,"fail":5,"rt_avg":45.67,"fail_rate":0.05}
```
...
**Peeka 优势**：
- JSON 输出，易于程序化处理
- 与 `jq`、Prometheus 等工具无缝集成
- 轻量级设计，性能影响极小

**Arthas 优势**：
- 支持条件过滤（如 `params[0] > 100`）
- 文本输出更适合人类阅读
- 内置 Web UI

...
```bash
# 查看内存概览（默认 action）
peeka memory --pid 12345

# 或显式指定 action
peeka memory --pid 12345 --action overview
```
...
```bash
# 使用默认深度（25 层调用栈）
peeka memory --pid 12345 --action start

# 自定义调用栈深度（1-50）
peeka memory --pid 12345 --action start --nframe 50
```
...
```bash
peeka memory --pid 12345 --action stop
```
...
```bash
# 正确流程：先导出，再停止
peeka memory --pid 12345 --action dump --filename production_snapshot
peeka memory --pid 12345 --action stop
```
...
peeka memory --pid 12345 --action top

# 查看 top 50 分配
peeka memory --pid 12345 --action top --limit 50

# 按文件名分组（查看哪个模块占用多）
peeka memory --pid 12345 --action top --group-by filename --limit 30
```
...
```bash
peeka memory --pid 12345 --action top --group-by filename --limit 10
```
...
```bash
# 自动生成文件名（时间戳）
peeka memory --pid 12345 --action dump

# 指定文件名
peeka memory --pid 12345 --action dump --filename my_snapshot

# 带路径遍历保护（自动提取 basename）
peeka memory --pid 12345 --action dump --filename "../etc/passwd"
...
```bash
# 查看 top 20 对象类型（默认）
peeka memory --pid 12345 --action gc

# 查看 top 50 对象类型
peeka memory --pid 12345 --action gc --limit 50
```
...
# 4. 导出第一个快照
peeka memory --pid 12345 --action dump --filename snapshot_before

# 5. 继续等待
sleep 300

# 6. 导出第二个快照
peeka memory --pid 12345 --action dump --filename snapshot_after

# 7. 查看 top 分配
peeka memory --pid 12345 --action top --limit 30
...
# 3. 查看内存热点（按文件分组）
peeka memory --pid 12345 --action top --group-by filename --limit 20

# 4. 查看具体代码行（按行号分组）
peeka memory --pid 12345 --action top --group-by lineno --limit 50
...
# 启动追踪（首次）
peeka memory --pid $PID --action start

# 每小时导出快照
while true; do
  timestamp=$(date +%Y%m%d_%H%M%S)
  peeka memory --pid $PID --action dump --filename "snapshot_$timestamp"
  sleep 3600
done
```
...
1. **按需启动**：
   ...
   peeka memory --pid 12345 --action start
   sleep 300  # 5 分钟
   peeka memory --pid 12345 --action dump --filename snapshot
   peeka memory --pid 12345 --action stop
   ```
...
3. **低峰期使用 gc**：
   ```bash
   # gc 操作开销较大，建议在低峰期执行
   peeka memory --pid 12345 --action gc --limit 30
   ```
...
4. **定期导出快照**：
   ...
   # 每 1 小时导出一次，用于趋势分析
   while true; do
     peeka memory --pid 12345 --action dump --filename "snapshot_$(date +%H)"
     sleep 3600
   done
   ```
...
peeka memory --pid $PID --action overview | \
  jq -r '.rss_bytes' | \
  while read rss; do
    if [ $rss -gt $ALERT_THRESHOLD ]; then
      ...
      peeka memory --pid $PID --action start
      sleep 30
      peeka memory --pid $PID --action dump --filename "alert_$(date +%s)"
      peeka memory --pid $PID --action stop
    fi
  done
```

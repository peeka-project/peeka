...
peeka stack -p 12345 "myapp.orders.process_order"
```
...
```bash
# 限制栈深度为 20 层，避免输出过长
peeka stack -p 12345 "myapp.utils.recursive_func" --depth 20
```
...
peeka stack -p 12345 "myapp.auth.check_permission" \
...
peeka stack -p 12345 "myapp.db.execute_query" -n 100 | \
...
```bash
# 追踪 API 入口函数的调用链
peeka stack -p 12345 "myapp.api.handle_request"
```
...
```bash
peeka stack [--pid PID | --name NAME] <pattern> [options]
```
...
```bash
# 只看第一次调用
peeka stack -p 12345 "myapp.func" -n 1

# 捕获 50 次样本
peeka stack -p 12345 "myapp.func" -n 50
```
...
```bash
# 仅当第一个参数大于 100 时捕获
peeka stack -p 12345 "myapp.func" --condition-express "params[0] > 100"

# 仅当参数个数大于 2 时捕获
peeka stack -p 12345 "myapp.func" --condition-express "len(params) > 2"

# 仅当用户 ID 为指定值时捕获
peeka stack -p 12345 "myapp.check_permission" \
  --condition-express "kwargs.get('user_id') == 999"

# 复合条件
peeka stack -p 12345 "myapp.process" \
...
```bash
# 只看最近 3 层调用者
peeka stack -p 12345 "myapp.func" --depth 3

# 深度递归分析（最多 30 层）
peeka stack -p 12345 "myapp.recursive_func" --depth 30
```
...
```bash
peeka stack -p 12345 "myapp.pricing.calculate_price"
```
...
```bash
peeka stack -p 12345 "myapp.utils.process_value" \
...
```bash
peeka stack -p 12345 "myapp.db.execute_query" --depth 3
```
...
```bash
peeka stack -p 12345 "myapp.cache.get" -n 1
```
...
```bash
peeka stack -p 12345 "myapp.logger.log" -n 100 | \
  jq -r '.stack[0] | "\(.filename):\(.lineno)"' | \
...
```bash
peeka stack -p 12345 "myapp.shared.resource.access" -n 50 | \
...
peeka stack -p 12345 "myapp.payment.charge" \
  --condition-express "params[0] <= 0" \
  -n 20
...
```bash
# 步骤 1：捕获 200 次调用的调用栈
peeka stack -p 12345 "myapp.db.query" -n 200 > query_stacks.jsonl
...
peeka stack -p 12345 "myapp.tree.traverse" --depth 50 -n 10 > recursion.jsonl
...
**管理建议**：
- 输出到文件：`peeka stack ... > output.jsonl`
- 定期清理输出文件
...
- 可以用 `jq` 过滤特定线程：
  ```bash
  peeka stack -p 12345 "myapp.func" | \
...
- 可能看到大量 `asyncio` 内部栈帧，使用 `jq` 过滤即可：
  ```bash
  peeka stack -p 12345 "myapp.async_func" | \
...
```bash
# 查看当前所有追踪任务
peeka watch -p 12345 --action status

# 停止特定任务
peeka watch -p 12345 --action stop --watch-id watch_001
```
...
```bash
# 终端 1：追踪调用栈
peeka stack -p 12345 "myapp.func" > stack.jsonl

# 终端 2：观测参数和返回值
peeka watch -p 12345 "myapp.func" > watch.jsonl
```
...
```bash
# 追踪 json.dumps 的调用栈
peeka stack -p 12345 "json.dumps" --depth 5

# 追踪 logging 模块的 info 方法
peeka stack -p 12345 "logging.Logger.info" --depth 3
```
...
```bash
peeka stack -p 12345 "myapp.db.query" -n 500 | \
  jq -r '.stack[0:3] | map("\(.filename):\(.lineno)") | join(" -> ")' | \
...
```bash
# 步骤 1：提取调用关系
peeka stack -p 12345 "myapp.func" -n 100 | \
...
```bash
peeka stack -p 12345 "myapp.views.index" --depth 20 | \
...
```bash
# 服务 A
peeka stack -p 12345 "service_a.process" | \
  jq --arg service "A" '. + {service: $service}' > trace_a.jsonl

# 服务 B
peeka stack -p 67890 "service_b.handle" | \
...
```bash
# 脚本示例（需要自行实现）
peeka stack -p 12345 "myapp.api.endpoint" -n 1000 | \
  jq -r '.stack[0] | "\(.filename):\(.lineno)"' | \
  sort | uniq -c | \
...
**Peeka (Python)**：
```bash
peeka stack -p 12345 "myapp.MyClass.myMethod" -n 10 \

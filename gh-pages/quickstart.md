...
1. **进程选择** - 自动发现并选择目标进程
2. **Dashboard 仪表盘** (d) - 实时显示进程信息
3. **Watch 观测视图** (w) - 交互式观测函数
4. **Trace 追踪视图** (t) - 可视化调用树
5. **Stack 调用栈视图** (s) - 调用栈追踪
6. **Monitor 监控视图** (m) - 性能监控
7. **Logger 日志视图** (l) - 日志管理
8. **Memory 内存视图** (e) - 内存分析
9. **Inspect 检查视图** (i) - 对象检查
10. **Threads 线程视图** (h) - 线程分析
11. **Top 热点视图** (F8) - 函数性能采样

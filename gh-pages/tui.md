...
| 快捷键     | 功能                | 说明                      |
|---------|-------------------|-------------------------|
| `d`     | Dashboard 视图     | 切换到仪表盘                  |
| `w`     | Watch 视图         | 切换到函数观测                 |
| `t`     | Trace 视图         | 切换到调用链追踪                |
| `s`     | Stack 视图         | 切换到调用栈捕获                |
| `m`     | Monitor 视图       | 切换到性能监控                 |
| `e`     | Memory 视图        | 切换到内存分析                 |
| `l`     | Logger 视图        | 切换到日志管理                 |
| `i`     | Inspect 视图       | 切换到对象检查                 |
| `h`     | Threads 视图       | 切换到线程管理                 |
| `F8`    | Top 视图           | 切换到性能分析器                |
...
使用单字母快捷键在视图间快速切换，无需返回 Dashboard：
...
1. 在 Watch 视图启动观测（按 `w`，输入命令，按 Enter）
2. 切换到 Monitor 视图启动监控（按 `m`，输入命令，按 Enter）
3. 切换到 Top 视图启动性能分析（按 `F8`，输入命令，按 Enter）
4. 在各视图间切换查看实时数据
...
按 `?` 查看帮助，按 `d/w/t/s/m/e/l/i/h/F8` 切换视图，开始你的诊断之旅！

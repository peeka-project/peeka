...
|------|---------------|----------------|------|
| List loggers | ✅ `logger` | ✅ `logger list` | Consistent |
| Query logger | ✅ `logger -n name` | ✅ `logger get --logger name` | Slightly different syntax |
| Modify level | ✅ `logger --name x --level DEBUG` | ✅ `logger set --logger x --level DEBUG` | Consistent |
| Wildcard filter | ✅ | ✅ `--pattern` | Consistent |
| Level options | TRACE, DEBUG, INFO, WARN, ERROR | DEBUG, INFO, WARNING, ERROR, CRITICAL | Python standard levels |
| Modify appender | ✅ | ❌ | Peeka doesn't support |
| View handler | ✅ | ❌ | Peeka doesn't support |
...
- fnmatch pattern matching, more flexible
- Seamless integration with tools like `jq`

**Arthas Advantages**:
- Support modifying appender/handler
- Support viewing complete logger configuration (including handler, formatter)
- Built-in Web UI

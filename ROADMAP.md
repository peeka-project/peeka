...
- `Safety` Reset cleanup error visibility
  `ResetCommand._reset()` now merges per-handler cleanup errors into the
  response payload; CLI returns exit code 2 and TUI trace/stack views log
  warnings when partial cleanup failures occur.
- `Safety` Probe-type metadata registry
  `ProbeContext` exposes `register_probe_type()` and `managed_types()` so
  `top` and future streaming probes no longer require hardcoded strings at
  multiple call sites.
- `Safety` Chained exit-path cleanup hooks
  `PeekaAgent` installs additive handlers for SIGINT, SIGHUP, and
  `threading.excepthook` without overriding user-installed handlers, ensuring
  cleanup runs on additional interpreter exit paths.

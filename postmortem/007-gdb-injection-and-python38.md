...
| # | 事故 | 严重级别 | 日期 |
|---|------|----------|------|
| [#4](#事故-4python-38-中-gdb-注入创建线程后永不调度) | Python 3.8 中 GDB 注入创建线程后永不调度 | SEV-未标注（原文未提供） | 2026-03-24 |
| [#3](#事故-3python-38-gdb-attach-频繁超时超时预算不足) | Python 3.8 GDB attach 频繁超时（超时预算不足） | SEV-1 | 2026-03-20 |
| [#2](#事故-2gdb-注入类型转换错误触发-invalid-cast) | GDB 注入类型转换错误触发 Invalid cast | SEV-1 | 2026-03-20 |
| [#1](#事故-1__name__-检查导致-gdb-exec-注入后-agent-未初始化) | `__name__` 检查导致 GDB exec 注入后 Agent 未初始化 | SEV-1 | 2026-02-05 |
